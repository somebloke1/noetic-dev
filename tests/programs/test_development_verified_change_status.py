...
class DevelopmentVerifiedChangeStatusTest(unittest.TestCase):
    ...
    def test_compatibility_guard_rejects_overclaims_and_bad_types(self) -> None:
        cases: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
            ...
            ("wrong subject program", lambda data: data["subject"].__setitem__("program_id", "other.program")),
            ("wrong subject program version", lambda data: data["subject"].__setitem__("program_version", "v2")),
            ("wrong subject projection state", lambda data: data["subject"].__setitem__("projection_state", "running")),
            ("wrong source trace version", lambda data: data["subject"]["source"].__setitem__("trace_version", "wrong")),
            ("wrong source projection version", lambda data: data["subject"]["source"].__setitem__("projection_version", "wrong")),
            ("wrong source projection state", lambda data: data["subject"]["source"].__setitem__("projection_state", "running")),
            ("binding readiness smuggled", lambda data: data["subject"]["source"]["binding"].__setitem__("readiness", "ready")),
            ("binding command smuggled", lambda data: data["subject"]["source"]["binding"].__setitem__("current_command_authority", "actor-x")),
