...
This repository slice adds the canonical machine policy at `config/model-policy.json`, the strict schema at `governance/schemas/model-policy.schema.json`, contract tests, a broker-local route/outcome evidence lifecycle for protected review invocations, an OpenCode routed-session contract at `config/opencode-session-policy.json`, and a broker route-evidence contract at `governance/schemas/route-evidence.schema.json`. The OpenCode contract is `runtime_adapter_ready=false`; this slice deliberately does not claim live OpenCode, protected Pi, embedding, or ASR adapters are fully migrated, and it does not claim production deployment or external genus-router service operation.
...
- Broker route/outcome evidence: protected review attempts follow Terra -> Sol -> Luna candidate order, record every broker-local outcome, preserve high reasoning, bind every model reference to the local LiteLLM `/v1/responses` endpoint, reroute after recorded failure, and preserve exhausted terminal failure evidence.

Missing runtime adapters remain open work under Issue #29. This policy must not be cited as evidence that ASR readiness, embedding migration, live OpenCode routing, live broker genus-router route/outcome reporting, external genus-router service operation, production deployment, or protected Pi routing has completed.
