...
| AR-10 | A successful result binds repository, PR number, exact base/head SHAs, route decision and attempts, endpoint, model, reasoning, sophistication, availability, reviewed-diff digest, prompt digest, verdict, summary, and findings. Every invocation outcome is recorded. `changes-needed` fails the workflow. | Result-schema, failure-reroute, outcome, and requester exit-code tests. |
