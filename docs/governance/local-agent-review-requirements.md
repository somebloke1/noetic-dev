...
| AR-05 | Request size, file count, diff size, final assembled prompt size, Git command runtime, stdout, stderr, LiteLLM response, and model output are bounded. The broker rejects an over-limit assembled prompt before routing, so every returned prompt fits the routed model-input contract. Limit violations terminate work and fail closed. The prompt is sent only in the authenticated LiteLLM request body, never argv. | Boundary and over-boundary probes, timeout probe, and maximum-size assembled-prompt probe. |
...
| AR-10 | A successful result binds repository, PR number, exact base/head SHAs, the finite attempt contract, the shared schema-v1 route-evidence object, endpoint, model, reasoning, sophistication, availability, reviewed-diff digest, prompt digest, verdict, summary, and findings. Every attempt includes the complete routed decision, its separately captured READY-probe evidence, and one confirmed aggregate recorded outcome. The reviewer prompt requires blocking findings to honor explicit repository threat and acceptance boundaries; adjacent hardening remains residual. `changes-needed` fails the workflow. | Result-schema, route-evidence mutation, failure-reroute, outcome, prompt-boundary, and requester exit-code tests. |
