...
**Access-controlled source SHA-256:** `3d1cd70fccd61db2c1af7a0bb220741ced0c80a207018851cd6d1c7f49a83f5c`
**Publication status:** redacted public index

This document is the public, non-secret commitment and resumption contract for
the full local forensic inventory used by `ROADMAP.md`. The access-controlled
source remains outside Git. Its digest binds the exact worktree paths, branch and
HEAD identities, status rows, stash parentage and subjects, additional retained
heads, repeated configuration paths, executable locations, process observations,
and system-unit observations without publishing those workstation details.
...
| Stashes | 11 identities with base/additional-parent bindings in the access-controlled snapshot | Do not apply, pop, drop, rewrite, or count as integration evidence without a stash-specific issue and fresh QA |
| Additional retained heads | 12 identities in the access-controlled snapshot | Retain until reachability, ownership, replacement and QA are adjudicated |
...
| Client/runtime observation | Versions and capability conclusions are projected in `DECISIONS.md`; local executable/config paths, process IDs and unit observations remain only in the access-controlled snapshot | Point-in-time readiness evidence only; never authorization to edit user-global configuration |
...
## 3. Privacy and verification boundary

The access-controlled source may be compared against the published digest by an
authorized local operator. It must not be copied into issues, pull requests,
logs, public artifacts or generated documentation. Reviewers can verify the
public contract from the aggregate counts and digests above; item-specific work
can verify a narrowly disclosed opaque identity through its owning issue.
