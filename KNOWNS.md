...
<!-- governance-crud:start id=k-20260711-0008 -->
## k-20260711-0008: Forward APM donor exposes both strong deterministic invariants and redesign targets

- Ledger: knowns
- Status: verified
- Repository: /home/dgk/workspace/synthesis-worktrees/issue-3-controller-spec
- Created: 2026-07-11
- Updated: 2026-07-11
- Tags: controller,apm,donor,invariants,failure-lessons,verified
- Confidence: 0.92

Verified 2026-07-11 against `/home/dgk/workspace/noetic-pi-docker` at commit `30de068` and historical `/home/dgk/workspace/pi2`. Strong invariants: strict procedure parsing; typed dependency roles; ordered work units/waves; per-wave QA/remediation and commit proof; durable QA cases/findings/adjudication; scoped contract amendments; variant worktree isolation; explicit readiness/runtime classifications; bounded recovery; out-of-sequence rejection; transition events; independent agent/runtime roles. Redesign targets: generic state mutation and event append are not atomic; statuses/readiness/actionability are distributed across independently mutable rows and complex repair projections; ordinary routing contains correlation-to-agent fallback; recovery relies on process liveness/idle timestamps rather than durable leases/fencing; wave-only topology is legible but restrictive; git/workspace/spawn concerns are embedded in controller modules; compatibility and legacy amendment paths remain; active-limbo and stale-bookkeeping classifiers exist because lifecycle state can drift from lawful actionability. Therefore the superior controller should preserve conformance evidence while replacing donor structure with a versioned transition kernel, append-only causal journal plus snapshots, idempotent outbox, typed authority/dependencies, deterministic DAG/barrier scheduler, fenced attempts/workspaces, and portable effect ports.
<!-- governance-crud:end id=k-20260711-0008 -->
