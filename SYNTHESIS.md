...
3. **Wire the substrate — mandatory selection and one access boundary.** Make genus-router the mandatory model-selection MCP for every generative/coding task, with selection varying by honestly classified work sophistication. Make **LiteLLM the universal model-access boundary** beneath it, including calls mediated by Pi, OpenCode, brokers, llama.cpp, and Ollama-backed modalities. The active generative set is Sol, Terra, Luna, and narrowly governed high-value Fable 5, ordered `Sol | Fable > Terra > Luna`; Snowflake Arctic remains the embedding route and Qwen3-ASR the ASR route. Direct provider access is forbidden (dec-20260713-0010; `docs/model-routing-policy.md`).
