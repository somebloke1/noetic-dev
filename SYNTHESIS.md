...
1. **Canonicalize the notation (form first).** *Ratified* — `docs/cognitive-backbone.md` is the canonical `P1–P4` reference, with the imperative gloss for prose and ECN's `^!`/`^?` retained only as optional modality annotation (dec-20260718-0002). Every project references that one file instead of re-deriving the form.
...
3. **Wire the substrate — access and selection at their own layers.** Genus-router is the model-*selection* MCP for telos-driven work; **LiteLLM is the mandatory model-*access*/normalization body** beneath it for every noetic-dev invocation, including embeddings and ASR (dec-20260718-0002). Infrastructure beneath LiteLLM may remain provider-specific, but noetic-dev callers receive no direct endpoint escape hatch. Register the 2x3090-backed models through LiteLLM and finish this.
