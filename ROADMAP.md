...
questions, the privacy-preserving retained-residue index
`docs/workspace-residue-inventory-2026-08-01.md` SHA-256
`833e6a1939b5e0a0d230570d64431e5e133f0ffb2cb7da4400862b16db3bd8bb`, its
historical full source commitment SHA-256
...
an unworked promise. The historical full snapshot is committed by
SHA-256 `3d1cd70fccd61db2c1af7a0bb220741ced0c80a207018851cd6d1c7f49a83f5c`.
Its public aggregate counts, opaque digests, blockers and resumption criteria are
bound by `docs/workspace-residue-inventory-2026-08-01.md` SHA-256
`833e6a1939b5e0a0d230570d64431e5e133f0ffb2cb7da4400862b16db3bd8bb` and are
incorporated into every retention row below. Absolute workstation paths, account
identity, process IDs and full branch/worktree topology are omitted from the current tree; their prior publication is acknowledged.

| Item | Exact known state | Step-4 action | Commit/PR/merge or retention reason |
|---|---|---|---|
| noetic-dev constitutional/inventory work in root | `main@29196a67349537d6f8a8a711df11b86da0430857` with modified `ABEYANT_INTENTIONS.md`, `DECISIONS.md`, `KNOWNS.md`, `OPEN_QUESTIONS.md`, `README.md`, and `SYNTHESIS.md`; untracked `docs/original-intent-traceability.md`, `docs/full-portfolio-capability-map.md`, this `ROADMAP.md`, and the residue appendix | Issue #80 transferred and reconciled those ten paths from `origin/dev@1195c88f6160e4e15f430fd3f67fcb5eda19f559`, expanded scope to `AGENTS.md` and `docs/cognitive-backbone.md` when QA proved those projections required reconciliation, and reached exactly 13 paths when hosted review required a changed, public-redacted `initial-user-msg.md`; the already-listed residue index path was minimized in place. Consumed predecessors are exactly `da96142df2e6025a6ef7ba027f98fa167697a542`/tree `906a7240eca87d029f91863730d8eb87af0eb96b`, `2f21693128cb12404173666ccfc4f5eba7fbc13a`/tree `f0881741fdedcb03c04f83d650330983b026798e`, `1f946ae6b3406ff2154cdab3b40e72c5f8a18d7b`/tree `83a9fbb58a883217e7361b927b05ff8c1456fb56`, `9c14c18612fcb6191002767d0cd6311331b553fc`/tree `2670f55e1338110d35b2855205bf1f1b9910196b`, and `52601832a278a59136427c255b21eb43a77ae955`/tree `ad8fb28ff19db579b22c7a4c538e755b441e4703`; each received exactly one `REJECTED` QA and may not advance. Generation `4d56011c941785b62d3cf1ad4b486924df8ab00c`/tree `0bbd6b0155c26c3fb149feb6600f6c77df6c92b0` received one `ACCEPTED` independent read-only QA, but hosted Agent Review run `30698087156` returned changes-needed on the public source/privacy boundary, so it is consumed for integration. Generation `c89deedcfc09f425a90230a934c57a31bad3a48f`/tree `c4fd9b30798ccfb817682dfb79f13622531f48f1` received exactly one `REJECTED` QA because the live PR remained bound to the consumed parent and three rows still misdescribed the public residue index. Generation `2f7d0522374f639df6235592b200e7c5bcf1d3d5`/tree `c8f138cf8936e7c3055dadba072b29d02b567a43` received exactly one `REJECTED` QA and hosted Agent Review run `30698838897` changes-needed for 13th-path accounting, source fidelity/citations, and historical-publication honesty. Freeze only a fresh successor, publish it only as verification evidence when live PR/check binding is required, then obtain exactly one independent QA before merge | Every frozen generation has exactly one QA. A commit cannot embed its own stable SHA/tree, so live issue #80 is the sole mutable record binding the checked-out successor identity and pending/final QA state; Git ancestry and this row bind all consumed predecessors. Rejected generations are never amended or retrospectively accepted. The final PR may retain their lineage because squash integration publishes only the independently accepted successor tree |
...
| noetic-pi and its worktrees | Donor head `683b53b06714d0aadd49c5852140b708c5abba69`; the historical full source records 16 worktrees including one missing registration, five stash identities, every full HEAD, and each dirty-manifest digest; the public index commits only aggregate categories and source digest | No commit, merge, push, stash action, cleanup, prune, or export revival | Pin source invariants/fixtures read-only; resume only through a bounded extraction issue in the selected component home, which may disclose only the necessary opaque item identity and status-specific rule |
...
| saeproj and related research | `pi2-governance-adoption@fcc4bc465cd82256cec3d6cfc7e6958510291c9d`, dirty-manifest SHA-256 `e8ec071a5f984a2b5ed17b42f7cb4479e22ac568dd3df5be847bf86bdda32f52`, stash `8641ab98e253c39d845034ef48e6d502e62b3aed`, no configured upstream; the historical full source records other retained heads | No bulk integration or stash action. First execute the T8 target-governance issue; create research-owned evidence issues only when geometry, benchmark, and governance changes can be separated | Research remains active/non-gating; preserve stash, v3 authority, untracked v4 uncertainty, and negative results; resume under the T8 evidence package |
...
| ContextForge PR 392 and root | PR conflicts; root `codex/issue-389-cf-catalog-service-truth@834badb9f64371539e44a0f4f674c56b7ee7d0cf` is two local commits ahead with dirty-manifest SHA-256 `7e777b556cb3382c87fdd1101bdf1df5c023c88d1a0e5c8f02c5b7f807321939` and stale topology | Retain untouched and separate from noetic-dev; comment/close only under its own issue authority | Optional transport estate, not a noetic-dev integration candidate; exact ahead commits and status rows remain in the historical full source |
| ContextForge stashes, branches, detached/terminal worktrees, repeated `.codex` changes | The historical full source records every full path/branch/HEAD/status and stash identity; the public index publishes only aggregate counts, source digest, and the 23-row configuration-group SHA-256 `ea5fa0c00da360a4f4a542a9f828934506e081a1178dcc57467d436cded83d6e` | Retain; do not prune, apply/drop stash, reset, rebase, force-push, or delete | Each item resumes only through its owning issue plus narrowly disclosed opaque identity and reachability/ownership/QA checks; repeated status is not cleanup authority |
| Client/global configs and runtime services | The historically published full source records local executable/configuration paths, versions, point-in-time processes and unit observations; the current public index exposes only capability conclusions and its source digest | Read-only evidence only during Step 4; do not publish or edit/open/hash config values and do not rely on stale process observations | User-global/runtime state is outside repository delivery; later adapter trials use isolated homes/config and refresh only their own necessary non-secret identity |
...
5. If independent acceptance requires exact live PR or hosted-check evidence,
   push the frozen SHA/tree only to its issue-linked verification PR, update that
   PR's exact contract, and run hosted checks. This publication is not acceptance
   or merge authority; the unchanged candidate remains fail-closed and unmerged.
   Otherwise keep the candidate local until QA.
6. Dispatch exactly one independent QA with the claim but not the implementer's
   self-assessment after all evidence required by that claim is observable;
   record task identity, transcript, and verdict.
7. On rejection, consume the generation and create one materially changed
   successor only if P3/P4 reassessment still warrants it.
8. Merge only when the exact unchanged candidate has accepted QA and all current
   hosted checks. If it was not already published for verification, push/open its
   linked PR now. Never merge a verification-only or stale-PR candidate.
